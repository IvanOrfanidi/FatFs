...
)
{
    ...
    csz =
        512UL *
        fp->fs
            ->csize; /* Cluster size in unit of byte (assuming 512 bytes/sector) */
    ...
    if(len == fp->fsize) {
        ...
        if(ncl == tcl) /* Is the file contiguous? */
            return clust2sect(fp->fs,
                fp->sclust); /* File is contiguous. Return the start sector */
    }
...
int main(void)
{
    ...
    if(!org) {
        printf("Function failed due to any error or insufficient contiguous "
               "area.\n");
        f_close(&fil);
        return 1;
    }
    ...
    ... dr = disk_write(
        fil.fs->drv, Buff, org, 1024); /* Write 512KiB from top of the file */
    ...
