...
)
{
    ...
    for(cc = 1; cc <= ncyc; cc++) {
        ...
        printf(
            " disk_ioctl(%u, GET_SECTOR_COUNT, 0x%08X)", pdrv, (UINT)&sz_drv);
        sz_drv = 0;
        ...
        if(sz_drv >= 128 + 0x80000000 / (sz_sect / 2)) {
            ...
            for(n = 0, pn(pns);
                pbuff[n] == (BYTE)pn(0) && n < (UINT)(sz_sect * 2);
                n++)
                ;
            if(n == (UINT)(sz_sect * 2)) {
                printf(" Data matched.\n");
...
int main(int argc, char* argv[])
{
    ...
    if(rc) {
        printf("Sorry the function/compatibility test failed. (rc=%d)\nFatFs "
               "will not work with this disk driver.\n",
            rc);
    }
    else {
